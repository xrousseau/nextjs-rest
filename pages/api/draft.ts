...
import { getArticle } from '../../lib/api-rest';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Fetch the article to check if the provided `slug` exists
  const article = await getArticle(slug as string, true);
  ...
  const url = `/articles/${article.fields.slug}`;
